<?php
/*
 * Template Name: Custom Events Irish Page
 * Description: Custom Events Irish Page
 */
?>
<?php get_header(); ?>
...
function showArtist(str) {
    if (str.length == 0) {
        ...
        xmlhttp.open("GET", "wp-content/themes/areview-child/artist_info2_irish.php?q=" + str, true);
        xmlhttp.send();
			}
	}
	</script>
...
<!--//Artists Information //////////////////////-->
<div class="event">Dé Máirt, 27 Nollaig</div>
<?php
...
foreach($results as $row) {
	...
	print ("<form action=wp-content/themes/areview-child/artist_info2_irish.php method=post>");
...
?>
<div class="event">Dé Céadaoin, 28 Nollaig</div>
<?php
...
?>
<div class="event">Déardaoin, 29 Nollaig</div>
<?php
...
?>
<div class="event">Dé hAoine, 30 Nollaig </div>
<?php
...
?>
<div class="event">Dé Satharn, 31 Nollaig</div>
<?php
...
?>
<div class="event">Dé Domhnaigh, 1 Eanáir</div>
<?php

global $wpdb;
$results = $wpdb->get_results("SELECT * FROM wp_artistsirishirish WHERE date LIKE '%1 Eanáir' ORDER BY artist_id ASC", ARRAY_A);
foreach($results as $row) {
          $artist_id = $row['artist_id'];
		  		$artist_name  = $row['artist_name'];
					$artist_info = $row['artist_info'];
